...
[the HTTP API for modifying collections](https://docs.c8db.com/latest/HTTP/Collection/Modifying.html).

## C8Database.createCollection

`C8Database.createCollection(String name, CollectionCreateOptions options) : CollectionEntity`
...
- **options**: `CollectionCreateOptions`
  ...
  - **isVolatile**: `Boolean`
    ...
    slightly faster than regular collections because C8DB does not enforce
  ...
  - **indexBuckets**: `Integer`
    ...
    but other index types might follow in future C8DB versions.
...
`C8Collection.create(CollectionCreateOptions options) : CollectionEntity`

Creates a collection with the given _options_ for this collection's name,
then returns collection information from the server.

Alternative for [C8Database.createCollection](#c8databasecreatecollection).
...
`C8Collection.load() : CollectionEntity`
...
`C8Collection.unload() : CollectionEntity`
...
## C8Collection.changeProperties

`C8Collection.changeProperties(CollectionPropertiesOptions options) : CollectionPropertiesEntity`
...
`C8Collection.rename(String newName) : CollectionEntity`
...
`C8Collection.truncate() : CollectionEntity`

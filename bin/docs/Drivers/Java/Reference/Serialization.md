...
registering additional `VPackModule`s on `C8DB.Builder`.

### Java 8 types

GitHub: https://github.com/c8db/java-velocypack-module-jdk8
...
```Java
C8DB c8DB = new C8DB.Builder().registerModule(new VPackJdk8Module()).build();
```

### Scala types

GitHub: https://github.com/c8db/java-velocypack-module-scala
...
```Scala
val c8DB: C8DB = new C8DB.Builder().registerModule(new VPackScalaModule).build
```

### Joda-Time

GitHub: https://github.com/c8db/java-velocypack-module-joda
...
```Java
C8DB c8DB = new C8DB.Builder().registerModule(new VPackJodaModule()).build();
```
...
[VelocyJack](https://github.com/c8db/jackson-dataformat-velocypack#within-c8db-java-driver)
which is based on [Jackson](https://github.com/FasterXML/jackson) working with
[jackson-dataformat-velocypack](https://github.com/c8db/jackson-dataformat-velocypack).
...
```Java
C8DB c8DB = new C8DB.Builder().registerModule(new VPackModule() {
  @Override
...
To use C8-internal fields (like \_id, \_key, \_rev, \_from, \_to) in your
...
`C8Util` from the method `util()` in `C8DB`, `C8Database`,
`C8Collection`, `C8Graph`, `C8EdgeCollection`or `C8VertexCollection`.

```Java
C8DB c8DB = new C8DB.Builder();
VPackSlice vpack = c8DB.util().serialize(myObj);
```

```Java
C8DB c8DB = new C8DB.Builder();
MyObject myObj = c8DB.util().deserialize(vpack, MyObject.class);
```

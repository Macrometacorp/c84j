...
public class UserAuthTest {
	...
	public UserAuthTest(final UserAuthParam param) {
		...
		c8DBRoot.createDatabase(DB_NAME);
		c8DBRoot.db(DB_NAME).createCollection(COLLECTION_NAME);
		c8DBRoot.db().grantAccess(USER_NAME, param.systemPermission);
		c8DBRoot.db(DB_NAME).grantAccess(USER_NAME, param.dbPermission);
		c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).grantAccess(USER_NAME, param.colPermission);
	...
	public void createDatabase() {
		try {
			if (Permissions.RW.equals(param.systemPermission)) {
				try {
					assertThat(details, c8DB.createDatabase(DB_NAME_NEW), is(true));
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.getDatabases(), hasItem(DB_NAME_NEW));
			} else {
				try {
					c8DB.createDatabase(DB_NAME_NEW);
					fail(details);
				} catch (final C8DBException e) {
				}
				assertThat(details, c8DBRoot.getDatabases(), not(hasItem(DB_NAME_NEW)));
			}
		} finally {
			try {
				c8DBRoot.db(DB_NAME_NEW).drop();
			} catch (final C8DBException e) {
			}
		}
	}
	...
	public void dropDatabase() {
		try {
			c8DBRoot.createDatabase(DB_NAME_NEW);
			if (Permissions.RW.equals(param.systemPermission)) {
				try {
					assertThat(details, c8DB.db(DB_NAME).drop(), is(true));
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.getDatabases(), not(hasItem(DB_NAME)));
			} else {
				try {
				...
				assertThat(details, c8DBRoot.getDatabases(), hasItem(DB_NAME));
			}
		} finally {
			try {
				c8DBRoot.db(DB_NAME_NEW).drop();
	...
	public void resetUserDBAccess() {
		try {
			...
			if (Permissions.RW.equals(param.systemPermission)) {
				try {
					c8DB.db(DB_NAME).resetAccess(USER_NAME_NEW);
				} catch (final C8DBException e) {
					fail(details);
				}
			} else {
	...
	public void grantUserCollcetionAccess() {
		try {
			...
			if (Permissions.RW.equals(param.systemPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).grantAccess(USER_NAME_NEW, Permissions.RW);
				} catch (final C8DBException e) {
					fail(details);
				}
			} else {
	...
	public void resetUserCollectionAccess() {
		try {
			...
			if (Permissions.RW.equals(param.systemPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).resetAccess(USER_NAME_NEW);
				} catch (final C8DBException e) {
					fail(details);
				}
			} else {
	...
	public void createCollection() {
		try {
			if (Permissions.RW.equals(param.dbPermission)) {
				try {
					c8DB.db(DB_NAME).createCollection(COLLECTION_NAME_NEW);
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME_NEW).getInfo(),
					is(notNullValue()));
			} else {
				try {
					c8DB.db(DB_NAME).createCollection(COLLECTION_NAME_NEW);
				...
				try {
					c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME_NEW).getInfo();
					fail(details);
				} catch (final C8DBException e) {
				}
			}
			...
			try {
				c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME_NEW).drop();
			} catch (final C8DBException e) {
			}
		}
	}
	...
	public void dropCollection() {
		try {
			c8DBRoot.db(DB_NAME).createCollection(COLLECTION_NAME_NEW);
			c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME_NEW).grantAccess(USER_NAME, param.colPermission);
			if (Permissions.RW.equals(param.dbPermission) && Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME_NEW).drop();
				} catch (final C8DBException e) {
					fail(details);
				}
				try {
	...
	public void seeCollection() {
		if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
			...
			try {
				final Collection<CollectionEntity> collections = c8DB.db(DB_NAME).getCollections();
				boolean found = false;
	...
	public void readCollectionInfo() {
		if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
				&& (Permissions.RW.equals(param.colPermission) || Permissions.RO.equals(param.colPermission))) {
			try {
				assertThat(details, c8DB.db(DB_NAME).collection(COLLECTION_NAME).getInfo(), is(notNullValue()));
			} catch (final C8DBException e) {
				fail(details);
			}
		} else {
			try {
				c8DB.db(DB_NAME).collection(COLLECTION_NAME).getInfo();
				fail(details);
			} catch (final C8DBException e) {
			}
		}
	...
	public void readCollectionProperties() {
		if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
				&& (Permissions.RW.equals(param.colPermission) || Permissions.RO.equals(param.colPermission))) {
			try {
				assertThat(details, c8DB.db(DB_NAME).collection(COLLECTION_NAME).getProperties(),
					is(notNullValue()));
			} catch (final C8DBException e) {
				fail(details);
			}
			...
			try {
				c8DB.db(DB_NAME).collection(COLLECTION_NAME).getProperties();
				fail(details);
			} catch (final C8DBException e) {
			}
		}
	...
	public void writeCollectionProperties() {
		if (Permissions.RW.equals(param.dbPermission) && Permissions.RW.equals(param.colPermission)) {
			try {
				assertThat(details, c8DB.db(DB_NAME).collection(COLLECTION_NAME)
						.changeProperties(new CollectionPropertiesOptions().waitForSync(true)),
					is(notNullValue()));
			} catch (final C8DBException e) {
				fail(details);
			}
			assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).getProperties().getWaitForSync(),
				is(true));
		} else {
			try {
				c8DB.db(DB_NAME).collection(COLLECTION_NAME)
						.changeProperties(new CollectionPropertiesOptions().waitForSync(true));
				fail(details);
			} catch (final C8DBException e) {
			}
	...
	public void readCollectionIndexes() {
		if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
				&& (Permissions.RW.equals(param.colPermission) || Permissions.RO.equals(param.colPermission))) {
			try {
				assertThat(details, c8DB.db(DB_NAME).collection(COLLECTION_NAME).getIndexes(), is(notNullValue()));
			} catch (final C8DBException e) {
				fail(details);
			}
		} else {
			try {
				c8DB.db(DB_NAME).collection(COLLECTION_NAME).getIndexes();
				fail(details);
			} catch (final C8DBException e) {
			}
		}
	...
	public void createCollectionIndex() {
		...
		try {
			if (Permissions.RW.equals(param.dbPermission) && Permissions.RW.equals(param.colPermission)) {
				try {
					final IndexEntity createHashIndex = c8DB.db(DB_NAME).collection(COLLECTION_NAME)
				...
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).getIndexes().size(), is(2));
			} else {
				try {
					final IndexEntity createHashIndex = c8DB.db(DB_NAME).collection(COLLECTION_NAME)
							.ensureHashIndex(Arrays.asList("a"), new HashIndexOptions());
				...
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).getIndexes().size(), is(1));
			}
		} finally {
			if (id != null) {
				c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).deleteIndex(id);
			}
		}
	}
	...
	public void dropCollectionIndex() {
		final String id = c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME)
				.ensureHashIndex(Arrays.asList("a"), new HashIndexOptions()).getId();
		try {
			if (Permissions.RW.equals(param.dbPermission) && Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).deleteIndex(id);
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).getIndexes().size(), is(1));
	...
	public void truncateCollection() {
		try {
			c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).insertDocument(new BaseDocument("123"));
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).truncate();
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).documentExists("123"),
					is(false));
			} else {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).truncate();
			...
			try {
				c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).truncate();
			} catch (final C8DBException e) {
			}
		}
	}
	...
	public void readDocumentByKey() {
		try {
			...
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& (Permissions.RW.equals(param.colPermission) || Permissions.RO.equals(param.colPermission))) {
				assertThat(details,
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).getDocument("123", BaseDocument.class),
					is(notNullValue()));
			} else {
				assertThat(details,
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).getDocument("123", BaseDocument.class),
			...
			try {
				c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).deleteDocument("123");
			} catch (final C8DBException e) {
			}
		}
	}
	...
	public void readDocumentByAql() {
		try {
			...
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& (Permissions.RW.equals(param.colPermission) || Permissions.RO.equals(param.colPermission))) {
				assertThat(details,
					c8DB.db(DB_NAME).query("FOR i IN @@col RETURN i",
						new MapBuilder().put("@col", COLLECTION_NAME).get(), new C8qlQueryOptions(), BaseDocument.class)
	...
	public void insertDocument() {
		try {
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).insertDocument(new BaseDocument("123"));
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).documentExists("123"),
	...
	public void updateDocumentByKey() {
		try {
			...
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).updateDocument("123",
						new BaseDocument(new MapBuilder().put("test", "test").get()));
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME)
	...
	public void updateDocumentByAql() {
		try {
			...
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).query("FOR i IN @@col UPDATE i WITH @newDoc IN @@col",
	...
	public void deleteDocumentByKey() {
		try {
			...
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).collection(COLLECTION_NAME).deleteDocument("123");
				} catch (final C8DBException e) {
					fail(details);
				}
				assertThat(details, c8DBRoot.db(DB_NAME).collection(COLLECTION_NAME).documentExists("123"),
	...
	public void deleteDocumentByAql() {
		try {
			...
			if ((Permissions.RW.equals(param.dbPermission) || Permissions.RO.equals(param.dbPermission))
					&& Permissions.RW.equals(param.colPermission)) {
				try {
					c8DB.db(DB_NAME).query("REMOVE @key IN @@col",
						new MapBuilder().put("key", "123").put("@col", COLLECTION_NAME).get(), new C8qlQueryOptions(),
						Void.class);
				} catch (final C8DBException e) {
					fail(details);
				}

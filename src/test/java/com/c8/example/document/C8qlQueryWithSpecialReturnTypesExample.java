...
package com.c8.example.document;
...
import com.c8.example.ExampleBase;
 ...
 *
 */
public class C8qlQueryWithSpecialReturnTypesExample extends ExampleBase {
	...
	public void aqlWithLimitQueryAsVPackObject() {
		...
		final C8Cursor<VPackSlice> cursor = db.query(query, bindVars, null, VPackSlice.class);
	...
	public void aqlWithLimitQueryAsMap() {
		final String query = "FOR t IN " + COLLECTION_NAME
				+ " FILTER t.age >= 20 && t.age < 30 && t.gender == @gender RETURN t";
		final Map<String, Object> bindVars = new MapBuilder().put("gender", Gender.FEMALE).get();
		final C8Cursor<Map> cursor = db.query(query, bindVars, null, Map.class);
		assertThat(cursor, is(notNullValue()));
		for (; cursor.hasNext();) {
	...
	public void aqlWithLimitQueryAsList() {
		final String query = "FOR t IN " + COLLECTION_NAME
				+ " FILTER t.age >= 20 && t.age < 30 && t.gender == @gender RETURN [t.name, t.gender, t.age]";
		final Map<String, Object> bindVars = new MapBuilder().put("gender", Gender.FEMALE).get();
		final C8Cursor<List> cursor = db.query(query, bindVars, null, List.class);
		assertThat(cursor, is(notNullValue()));
		for (; cursor.hasNext();) {

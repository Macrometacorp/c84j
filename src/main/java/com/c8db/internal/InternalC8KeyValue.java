...
public abstract class InternalC8KeyValue<A extends InternalC8DB<E>, D extends InternalC8Database<A, E>, E extends C8Executor>
    ...
    protected  <T> Request insertBlobKVPairsRequest(final Collection<BlobKeyValue> values) {
    ...
    protected Request getBlobKVPairRequest(final String key) {
        return request(db.tenant(), db.name(), RequestType.GET, PATH_API_KV, name, PATH_API_KV_PAIR, key)
                .putHeaderParam("Accept", ContentType.MULTIPART_FORM_DATA.getMimeType());
    }
    ...
    protected Request getBlobKVPairsRequest(final C8KeyValue.C8KVReadValuesOptions options) {
        final C8KeyValue.C8KVReadValuesOptions params = (options != null ? options : new C8KeyValue.C8KVReadValuesOptions());
            ...
            for (MultipartResponseBody.Item item : items) {
                if (item.getContentType().equals(ContentType.APPLICATION_JSON.getMimeType())) {
                    ...
                    if (next.get(C8ResponseField.ERROR).isTrue()) {
                        ...
                        VPackSlice valueVpack = next.get("value");
                        if (valueVpack != null) {
                            last.setValue(next.get("value").getAsString().getBytes(StandardCharsets.UTF_8));
                        }
                        docs.add(last);

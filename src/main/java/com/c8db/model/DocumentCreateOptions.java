...
public class DocumentCreateOptions extends MixinBase implements StrongConsistencyMixin<DocumentCreateOptions> {
    ...
    /**
     * @param returnOld Additionally return the complete old document under the
     *                  attribute old in the result. Only available if the
     *                  {@code overwrite} option is used.
     * @return options
     * @since ArangoDB 3.4
     */
    ...
    /**
     * @param overwrite If set to true, the insert becomes a replace-insert. If a
     *                  document with the same {@code _key} already exists the new
     *                  document is not rejected with unique constraint violated but
